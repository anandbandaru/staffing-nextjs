...
import PayrollsListToolbar from './payrollsListToolbar'
...
const PayrollList = () => {
    ...
    const getList = () => {
        setData({ data: [] });
        let apiUrl = APIPath + "/getpayrollsmmyyyy"
        fetch(apiUrl, {
            headers: {
                ...
                (result) => {
                    ...
                    if (result.error) {
                        ...
                        if (result.STATUS === "FAIL") {
                            ...
                            showSnackbar('success', "Payrolls Data fetched");
                    }
                    setApiLoading(false);
                },
                (error) => {
    ...
    const CustomDetailsComponent = (props) => {
        return (
            <>
                <GenericDetails ID={props.data.Id} operation="View" doLoading={false} moduleName="PAYROLLS" />
            </>
        );
    };
    const CustomEditComponent = (props) => {
        return (
            <>
                {/* <PayrollEdit ID={props.data.Id} operation="Edit" manualLoadData={manualLoadData} setApiLoading={setApiLoading} showSnackbar={showSnackbar} /> */}
            </>
        );
    };
    ...
    const [colDefs] = useState([
        ...
        { field: "payroll_MM_YYYY", filter: true, headerName: 'MM YYYY' },
