...
const InvoiceView = ({ operation, manualLoadData, invoiceNumber, employeeID, jobID, startDate, endDate, totalHours, status, jobType,
    ...
    showSnackbar, userNotes, vendorId, manualLoadDataWithMessage, performLoading, setPerformLoading, otherAmountFromDB, totalAmountFromDB }) => {
    ...
    const handleRateChange = (event) => {
        ...
        setLocalTotal(event.target.value * localHours);
    };
    const handleHoursChange = (event) => {
        setLocalHours(event.target.value);
        setLocalTotal(event.target.value * localRate);
    };
